...
const help_strings = if (@hasDecl(@import("root"), "help_strings")) 
    @import("help_strings") 
else 
    struct {
...
///
/// On Linux with OpenGL, this controls frame synchronization behavior.
/// When enabled, uses glFinish() for proper VSync. When disabled, uses
/// glFlush() for lower latency, which can help with slow typing on NVIDIA GPUs.
    ...
    pub fn formatEntryDocs(self: Keybinds, formatter: anytype, docs: bool) !void {
        ...
        while (iter.next()) |next| {
            ...
            if (docs) {
                ...
                // Only generate help if help_strings is actually available
                if (@typeInfo(help_strings.KeybindAction).@"struct".decls.len > 0) {
                    ...
                    // Fallback when help_strings is not available
                    try formatter.writer.writeAll("# ");
                    try formatter.writer.writeAll(name);
                    try formatter.writer.writeAll("\n");
                    try formatter.writer.writeAll("#   (Help text not available)\n");
                }
            }
