...
pub fn Renderer(comptime GraphicsAPI: type) type {
    return struct {
        ...
        pub fn drawFrame(
            ...
            // For OpenGL, respect the vsync config to control sync behavior
            // This helps with NVIDIA GPU performance issues
            const effective_sync = sync or (GraphicsAPI == renderer.OpenGL and self.config.vsync);
            defer frame_ctx.complete(effective_sync);
