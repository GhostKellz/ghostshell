...
///
/// NOTE: For OpenGL, we use glFlush() for async or glFinish() for sync.
/// This optimization is particularly important for NVIDIA GPUs where
/// glFinish() can cause significant stalls.
pub fn complete(self: *const Self, sync: bool) void {
    // Use glFlush for async operations (better for NVIDIA GPUs)
    // Only use glFinish when sync is explicitly requested
    if (sync) {
        gl.finish();
    } else {
        gl.flush();
