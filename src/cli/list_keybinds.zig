...
const TriggerList = std.ArrayList(Binding.Trigger);
...
const ChordBinding = struct {
    ...
    pub fn lessThan(_: void, lhs: ChordBinding, rhs: ChordBinding) bool {
        const lhs_len = lhs.triggers.items.len;
        const rhs_len = rhs.triggers.items.len;
        ...
        const lhs_count: usize = blk: {
            var count: usize = 0;
            for (lhs.triggers.items) |trigger| {
                if (trigger.mods.super) count += 1;
                if (trigger.mods.ctrl) count += 1;
                if (trigger.mods.shift) count += 1;
                if (trigger.mods.alt) count += 1;
            }
            break :blk count;
        };
        const rhs_count: usize = blk: {
            var count: usize = 0;
            for (rhs.triggers.items) |trigger| {
                if (trigger.mods.super) count += 1;
                if (trigger.mods.ctrl) count += 1;
                if (trigger.mods.shift) count += 1;
                if (trigger.mods.alt) count += 1;
        ...
        {
            for (lhs.triggers.items, rhs.triggers.items) |l_trigger, r_trigger| {
                const l_int = l_trigger.mods.int();
                const r_int = r_trigger.mods.int();
        ...
        for (lhs.triggers.items, rhs.triggers.items, 0..) |l_trigger, r_trigger, i| {
            const lhs_key: c_int = blk: {
                switch (l_trigger.key) {
                    .physical => |key| break :blk @intFromEnum(key),
                    .unicode => |key| break :blk @intCast(key),
                }
            };
            const rhs_key: c_int = blk: {
                switch (r_trigger.key) {
                    .physical => |key| break :blk @intFromEnum(key),
                    .unicode => |key| break :blk @intCast(key),
                }
            };

            if (i == lhs.triggers.items.len - 1 or i == rhs.triggers.items.len - 1) {
...
fn prettyPrint(alloc: Allocator, keybinds: Config.Keybinds) !u8 {
    ...
    for (bindings) |bind| {
        ...
        for (bind.triggers.items, 0..) |trigger, i| {
            ...
            if (i < bind.triggers.items.len - 1) {
    ...
    while (iter.next()) |bind| {
        ...
        switch (bind.value_ptr.*) {
            .leader => |leader| {
                ...
                for (sub_bindings) |*nb| {
                    try nb.triggers.insert(0, bind.key_ptr.*);
                }
            ...
            .leaf => |leaf| {
                var triggers = TriggerList.init(alloc);
                try triggers.append(bind.key_ptr.*);
