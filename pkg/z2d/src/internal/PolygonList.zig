...
pub fn deinit(self: *PolygonList, alloc: mem.Allocator) void {
    while (self.polygons.first) |node| {
        self.polygons.remove(node);
        const polygon_node = @fieldParentPtr("node", node);
        polygon_node.data.deinit(alloc);
        alloc.destroy(polygon_node);
    }
}

pub fn prepend(self: *PolygonList, alloc: mem.Allocator, poly: Polygon) mem.Allocator.Error!void {
    const first = self.polygons.first == null;

    const polygon_node = try alloc.create(PolygonNode);
    polygon_node.* = PolygonNode{ .data = poly };
    self.polygons.prepend(&polygon_node.node);
...
pub fn edgesForY(
    ...
    while (poly_) |poly| : (poly_ = poly.next) {
        const polygon_node = @fieldParentPtr("node", poly);
        var poly_edge_list = try polygon_node.data.edgesForY(alloc, line_y);
